...
/// A `Strategy` which always produces a single value and never simplifies.
///
/// This is useful for providing constant values in property tests, or as a
/// building block for more complex strategies.
...
impl<T: Clone + fmt::Debug> Strategy for Just<T> {
    ...
    fn new_tree(&self, _: &mut TestRng) -> NewTree<Self> {
        Ok(self.clone())
    }
}
...
/// A `Strategy` which always produces a single value and never simplifies.
///
/// If `T` is `Clone`, you should use `Just` instead. This is a generalization
/// of `Just` and works by calling the provided `Fn () -> T` in `.current()`
/// every time. This is not a very interesting strategy, but is required in
/// cases where `T` is not `Clone`.
///
